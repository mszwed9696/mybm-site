...
const filesToProcess = [
    ...
    'ListingScanPage.tsx',
    'MarketingQuizPage.tsx',
    'MarketingScorePage.tsx',
    'ProblemPage.tsx',
    'QuizPage.tsx',
    'SEOPage.tsx',
    'SocialCalculatorPage.tsx'
]; // excluding NotFound.tsx
...
filesToProcess.forEach(file => {
    ...
    // Patterns like <section className="pt-28...

    const heroRegexes = [
        /<section[^>]*?(?:pt-28|pt-32|section-padding|bg-gradient-hero|backgroundColor:\s*"#edf2f4")[^>]*>([\s\S]*?)<\/section>/i,
        /<div[^>]*?(?:pt-28|pt-32|section-padding|bg-gradient-hero|backgroundColor:\s*"#edf2f4")[^>]*>([\s\S]*?)<\/div>/i,
        // fallback: just find the first section
    ...
    // Dark mode color replacements globally
    content = content.replace(/backgroundColor:\s*"#edf2f4"/g, 'backgroundColor: "#141419"');
    content = content.replace(/backgroundColor:\s*"#fff"/g, 'backgroundColor: "#1a1a24"');
    ...
    content = content.replace(/color:\s*"#374151"/g, 'color: "rgba(244,244,245,0.8)"');
    content = content.replace(/color:\s*"#9ca3af"/g, 'color: "rgba(244,244,245,0.5)"');
